...
| 1.9 | Spring Batch 설계 문서 작성 | `10_batch/` 문서 3종 | 미정 | 1일 | 1.7, 2.5 | 작성 예정 |
| 1.10 | Vue 전환 로드맵 문서 작성 | `11_frontend-roadmap/` 문서 2종 | 미정 | 1일 | 2.1, 1.7 | 작성 예정 |
| **2** | **설계** | | | | | |
...
| 2.9 | 배치 수집 로그 테이블 설계 | batch_collection_log | 미정 | 0.5일 | 2.3 | 작성 예정 |
| **3** | **환경 구성** | | | | | |
| 3.1 | Spring Boot 프로젝트 초기화 | build.gradle | 미정 | 0.5일 | 2.5 | 작성 예정 |
| 3.2 | DB 연결 및 스키마 적용 | schema.sql 실행 결과 | 미정 | 0.5일 | 2.4, 3.1 | 작성 예정 |
| 3.3 | MyBatis 설정 | MyBatisConfig.java | 미정 | 0.5일 | 3.2 | 작성 예정 |
| 3.4 | Spring Batch 설정 | BatchConfig.java, JobLauncher 설정 | 미정 | 0.5일 | 3.1 | 작성 예정 |
| 3.5 | 공통 에러 처리 구현 | GlobalExceptionHandler.java | 미정 | 0.5일 | 3.1 | 작성 예정 |
| **4** | **백엔드 구현 — 필수 기능** | | | | | |
| 4.1 | 행정구역 코드 데이터 수집 설계 | region_code 수집 방안 | 미정 | 0.5일 | 1.9 | 작성 예정 |
| 4.2 | 주택 거래 수집 Job 구현 | houseDealCollectJob | 미정 | 2일 | 3.4, 2.9 | 작성 예정 |
| 4.3 | 배치 관리자 API 구현 | AdminBatchController, BatchJobService | 미정 | 1일 | 4.2 | 작성 예정 |
| 4.4 | 주택 검색·상세 조회 API | HouseController, HouseService, HouseMapper | 미정 | 2일 | 4.2 | 작성 예정 |
| 4.5 | 회원 가입·조회·수정·탈퇴 API | MemberController, MemberService, MemberMapper | 미정 | 1.5일 | 3.5 | 작성 예정 |
| 4.6 | 로그인·로그아웃·인증 API | AuthController, AuthService, AuthInterceptor | 미정 | 1일 | 4.5 | 작성 예정 |
| 4.7 | 관심 지역 API | FavoriteController, FavoriteService, FavoriteMapper | 미정 | 1일 | 4.6 | 작성 예정 |
| 4.8 | 공지사항 API | NoticeController, NoticeService, NoticeMapper | 미정 | 1일 | 4.6 | 작성 예정 |
| **5** | **백엔드 구현 — 추가 기능** | | | | | |
| 5.1 | 주변 상권 정보 API | CommercialController, CommercialService | 미정 | 1.5일 | 4.4 | 작성 예정 |
| 5.2 | 주변 환경 정보 API | EnvironmentController, EnvironmentService | 미정 | 1.5일 | 4.4 | 작성 예정 |
| **6** | **백엔드 구현 — 심화 기능** | | | | | |
...
| 7.2 | 주택 검색·결과 화면 | SCR-SEARCH, SCR-RESULT | 미정 | 2일 | 4.4, 7.1 | 작성 예정 |
| 7.3 | 매물 상세 화면 | SCR-DETAIL | 미정 | 2일 | 4.4, 7.2 | 작성 예정 |
| 7.4 | 로그인·회원 가입 화면 | SCR-LOGIN, SCR-SIGNUP | 미정 | 1일 | 4.5, 7.1 | 작성 예정 |
| 7.5 | 마이페이지·관심 지역 화면 | SCR-MYPAGE, SCR-FAVORITE | 미정 | 1.5일 | 4.7, 7.4 | 작성 예정 |
| 7.6 | 공지사항 화면 | SCR-NOTICE-LIST, SCR-NOTICE-DETAIL, SCR-NOTICE-FORM | 미정 | 1일 | 4.8, 7.1 | 작성 예정 |
...
| 7.9 | Vue 전환 준비 설계 | Vue Router/Pinia/API 계약 문서 | 미정 | 0.5일 | 1.10 | 작성 예정 |
| **8** | **통합 및 정리** | | | | | |
