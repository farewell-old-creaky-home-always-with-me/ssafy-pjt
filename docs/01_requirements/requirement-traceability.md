...
|------|-----------|
| 회원 테이블 | `member` |
| 행정구역 테이블 | `region_code` |
| 주택 기본 정보 테이블 | `house` |
| 주택 거래 이력 테이블 | `house_deal` |
| 관심 지역 테이블 | `favorite_area` |
| 상권 정보 테이블 | `commercial_area` |
| 환경 정보 테이블 | `environment_info` |
| 공지사항 테이블 | `notice` |
| 경로 탐색 테이블 | `route_node`, `route_edge` |
| 배치 수집 로그 테이블 | `batch_collection_log` |
...
- 모든 `REQ-*` 항목은 최소 1개의 화면 또는 운영 진입점, 1개의 API, 1개 이상의 DB 테이블과 연결한다.
- `REQ-HOUSE-001`, `REQ-ROUTE-001`처럼 배치 또는 알고리즘 의존성이 큰 항목은 표 외에 상세 추적 섹션을 유지한다.
- `Spring Batch`, `A*`는 구현 상세가 아니라 연계 지점으로만 추적한다.
- 화면 ID, API 경로, DB 테이블명은 각각 [screen-list.md](../03_ui/screen-list.md), [api-spec.md](../05_api/api-spec.md), [table-spec.md](../04_database/table-spec.md)와 동일해야 한다.
