...
| route_node 목록 | List\<RouteNode\> | 초기 구현에서 DB에서 한 번에 로드한 전체 노드 |
| route_edge 목록 | List\<RouteEdge\> | 초기 구현에서 DB에서 한 번에 로드한 전체 엣지 |
...
### 초기 구현 전략: 전체 노드와 엣지 메모리 로드

- `RouteMapper.findAllNodes()`로 전체 `route_node`를 조회한다.
- `RouteMapper.findAllEdges()`로 전체 `route_edge`를 조회한다.
- `RouteService`는 조회 결과를 메모리에 적재한 뒤 `AStarPathFinder`에 전달한다.
- 초기 범위에서는 구현 단순성과 예측 가능한 탐색 흐름을 우선한다.

### 개선 전략: `fromNodeId` 기준 인접 엣지 지연 조회

- 그래프 크기가 커져 메모리 사용량이나 초기 로드 시간이 부담되면 `RouteMapper.findEdgesByFromNodeId(fromNodeId)`를 사용한다.
- 이 방식은 현재 노드에서 인접한 엣지만 조회해 I/O와 메모리 사용량을 줄이는 방향의 최적화다.
- 다만 초기 구현 기준 문서와 클래스 설계에서는 전체 그래프 메모리 로드 전략을 기본으로 본다.
...
GET /api/routes?startLat={}&startLng={}&endLat={}&endLng={}
    └─ RouteController.findRoute()
         └─ RouteService.findRoute()
              ...
              ├─ RouteMapper.findAllNodes() → 그래프 노드
