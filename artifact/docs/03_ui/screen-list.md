...
- 현재 화면은 HTML/CSS/JavaScript로 구현한다.
- 모든 화면은 서버 사이드 페이지 렌더링에 의존하지 않고 REST API를 호출하는 구조를 기준으로 설계한다.
- 동일한 화면 ID는 향후 Vue Router 경로와 1:1 대응할 수 있도록 유지한다.
- 관리자 배치 화면은 향후 확장 대상이며 현재 사용자 UI 범위에는 포함하지 않는다.
...
| 푸터 | 전체 | 서비스 소개, 저작권 표시 |
| 지도 컴포넌트 | SCR-DETAIL, SCR-COMMERCIAL | Kakao Maps API 기반 위치 표시 |
